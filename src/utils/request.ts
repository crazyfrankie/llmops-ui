...
const baseFetchOptions = {
  method: 'GET',
  mode: 'cors',
  credentials: 'include', // 确保每次请求都携带Cookie（后端设置的Cookie会自动携带）
  headers: new Headers({
...
type FetchOptionType = Omit<RequestInit, 'body'> & {
  ...
  includeHeaders?: boolean // 是否包含响应头部信息
}
...
const baseFetch = <T>(url: string, fetchOptions: FetchOptionType): Promise<T> => {
  ...
  // 获取access_token并添加到Authorization头部
  // 权限策略：每次请求都需要携带access_token（放在Authorization头部）和Cookie
          ...
          if (json.code === httpCode.success) {
            ...
            if (fetchOptions.includeHeaders) {
              ...
              resolve({
                ...json,
                headers,
                access_token: res.headers.get('x-access-token') || '',
              })
            } else {
              resolve(json)
            }
