import { get, post, put, del, ssePost } from '@/utils/request'
import type {
  CreateAppRequest,
  GetAppResponse,
...
export const updateDraftAppConfig = (app_id: string, req: UpdateDraftAppConfigRequest) => {
  return put<BaseResponse<any>>(`/apps/${app_id}/draft-app-config`, { body: req })
}
...
export const updateDebugConversationSummary = (app_id: string, summary: string) => {
  return put<BaseResponse<any>>(`/apps/${app_id}/summary`, { body: { summary } })
}
...
export const debugChat = (
  ...
  return ssePost(`/apps/${app_id}/conversation`, { body: { query, image_urls } }, onData)
}

// 停止某次应用的调试会话
export const stopDebugChat = (app_id: string, task_id: string) => {
  return post<BaseResponse<any>>(`/apps/${app_id}/conversation/tasks/${task_id}/stop`)
}
...
export const getDebugConversationMessagesWithPage = (
  ...
  return get<GetDebugConversationMessagesWithPageResponse>(
    `/apps/${app_id}/conversation/messages`,
    { params: req },
  )
}
